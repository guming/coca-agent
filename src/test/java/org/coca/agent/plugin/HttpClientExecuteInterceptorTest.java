...
@PowerMockRunnerDelegate(TraceListRunner.class)
@PrepareForTest(HttpHost.class)
public class HttpClientExecuteInterceptorTest {
    @TraceListStoragePoint
    private TraceListStorage segmentStorage;
    ...
    @Mock
    private Object enhancedInstance;
...
//        HttpClientPluginConfig.Plugin.HttpClient.COLLECT_HTTP_PARAMS = true;
    ...
    public void testHttpClient() throws Throwable {
        httpClientExecuteInterceptor.before(enhancedInstance, null, allArguments, argumentsType, null);
        httpClientExecuteInterceptor.after(enhancedInstance, null, allArguments, argumentsType, httpResponse);

        Assert.assertThat(segmentStorage.getTraceList().size(), is(1));
        TraceList traceSegment = segmentStorage.getTraceList().get(0);

        List<TracingSpan> spans = TraceListHelper.getSpans(traceSegment);
    ...
    public void testHttpClientWithException() throws Throwable {
        httpClientExecuteInterceptor.before(enhancedInstance, null, allArguments, argumentsType, null);
        httpClientExecuteInterceptor.handleException(enhancedInstance, null, allArguments, argumentsType, new RuntimeException("testException"));
        httpClientExecuteInterceptor.after(enhancedInstance, null, allArguments, argumentsType, httpResponse);
    ...
    private void assertHttpSpanErrorLog(List<SpanLogData> logs) {
        assertThat(logs.size(), is(1));
        SpanLogData logData = logs.get(0);
        Assert.assertThat(logData.getLogs().size(), is(3));
...
//        assertNotNull(logData.getLogs().get(3).getValue());
    ...
    private void assertHttpSpan(TracingSpan span) {
